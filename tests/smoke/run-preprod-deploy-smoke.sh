...
#
# db-mcp-server — PS-PREPROD-DEPLOY-SMOKE service-facing smoke entry point (W28E-1854).
#
# Runs the durable browser smoke (PDS-001..PDS-013) from the SSOT monorepo app
# (cloud-dog-ai-ui-monorepo/apps/db-mcp/tests/e2e/preprod-deploy-smoke.spec.ts)
# against the DEPLOYED preprod db-mcp service, AFTER the final deployed digest
...
#   E2E_BASE_URL        default https://dbmcpserver0.cloud-dog.net
...
#                       dev.services.dbmcpserver0.web_password from
...
E2E_BASE_URL="${E2E_BASE_URL:-https://dbmcpserver0.cloud-dog.net}"
...
APP_DIR="${MONOREPO_ROOT}/apps/db-mcp"
...
if [[ ! -d "${APP_DIR}" ]]; then
  echo "ERROR: db-mcp app not found at ${APP_DIR}" >&2
  exit 2
fi
...
PW_PREPROD_SMOKE_OUTPUT="${RESULTS_DIR}" \
  pnpm --filter @cloud-dog/app-db-mcp exec playwright test --config playwright.preprod-smoke.config.ts
