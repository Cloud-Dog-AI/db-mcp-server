...
class FakeIndicesClient:
    ...
    def put_index_template(self, name: str, body: dict[str, object], **_kwargs):
        self.templates[name] = deepcopy(body)
        return {"acknowledged": True}
...
class FakeOpenSearch:
    ...
    def search(self, *, index: str, body: dict[str, object], size: int = 10, **_kwargs):
        ...
        for doc_id, source in docs.items():
            if not _matches(source, query):
                continue
            payload = {"_id": doc_id, "_index": target, "_source": deepcopy(source)}
            hits.append(payload)
        return {"hits": {"hits": hits[:size]}}

    def count(self, *, index: str, body: dict[str, object], **_kwargs):
        hits = self.search(index=index, body=body, size=1000)
        return {"count": len(hits["hits"]["hits"])}

    def index(self, *, index: str, body: dict[str, object], id: str | None = None, **_kwargs):
        target = "widgets" if index == "widgets_current" else index
        next_id = id or str(len(self.indices.indices[target]["documents"]) + 1)
        self.indices.indices[target]["documents"][next_id] = deepcopy(body)
        return {"_id": next_id}
    ...
    def update_by_query(self, *, index: str, body: dict[str, object], **_kwargs):
        target = "widgets" if index == "widgets_current" else index
        script = body["script"]["params"]
        updated = 0
        total = 0
        for doc in self.indices.indices[target]["documents"].values():
            if not _matches(doc, body["query"]):
                continue
            total += 1
            for key, value in script.get("set_values", {}).items():
                doc[key] = value
            for key in script.get("unset_fields", []):
                doc.pop(key, None)
            updated += 1
        return {"total": total, "updated": updated}

    def delete_by_query(self, *, index: str, body: dict[str, object], **_kwargs):
        target = "widgets" if index == "widgets_current" else index
        remaining = {}
        deleted = 0
        for doc_id, doc in self.indices.indices[target]["documents"].items():
            if _matches(doc, body["query"]):
                deleted += 1
            else:
...
def test_adapter_capabilities_and_catalogue_calls() -> None:
    connector = OpenSearchConnector(uri="http://localhost:9200")
...
def test_adapter_data_and_schema_operations() -> None:
    ...
    inserted = connector.create("test-cluster", "widgets_current", {"name": "gamma", "owner_id": "u3", "value": 3})
    assert inserted["document"]["_id"]
    updated = connector.update("test-cluster", "widgets", {"term": {"owner_id": "u3"}}, {"$set": {"value": 9}})
    ...
    assert connector.extract_relationships("test-cluster", "widgets")[0]["relationship_type"] == "reference_candidate"
